...
import { X, Globe, Clipboard, ArrowRight, Upload, Layout, Settings, Factory, RotateCcw } from 'lucide-react';
...
export const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose, visualSettings, onSaveSettings }) => {
  ...
  // NUEVA FUNCIÓN DE RESET CONECTADA AL BACKEND
  const handleFactoryReset = async () => {
    if (!confirm('⚠️ ¿ESTÁS SEGURO?\n\nEsto borrará TODOS los datos de producción y stock de la base de datos.\nEsta acción no se puede deshacer.')) {
      return;
    }
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-fade-in">
      <div className="bg-white dark:bg-slate-900 rounded-[3rem] shadow-2xl w-full max-w-4xl border border-gray-200 dark:border-slate-800 overflow-hidden flex flex-col h-[90vh]">
        ...
        <div className="p-10 space-y-12 overflow-y-auto flex-1">
          ...
          <section className="p-6 bg-red-600/5 border border-red-600/10 rounded-3xl mt-8">
             <div className="flex items-center justify-between">
                <div className="flex items-center gap-4 text-red-600">
                   <ArrowRight size={20} />
                   <div className="space-y-1">
                      <p className="text-xs font-black uppercase tracking-tight">Acciones Destructivas</p>
                      <p className="text-[10px] opacity-70">Usar solo si hay corrupción de datos o al inicio de jornada.</p>
                   </div>
                </div>
                <button 
                  onClick={handleFactoryReset}
                  disabled={isResetting}
                  className="px-6 py-3 bg-white border border-red-200 text-red-600 text-[10px] font-black uppercase hover:bg-red-600 hover:text-white transition-all rounded-xl flex items-center gap-2 disabled:opacity-50"
                >
                   {isResetting ? 'Borrando...' : <><RotateCcw size={14} /> Resetear Fábrica</>}
                </button>
             </div>
          </section>
