...
// Utilidad para redondeo de precisión industrial
...
const processIncomingData = (data: any) => {
  ...
  data.zonas.forEach((zona: any) => {
    // Normalización: Si el código es 0, null, undefined o vacío, se trata como '0'
    let agentCodeRaw = String(zona.codigo_agente !== undefined && zona.codigo_agente !== null ? zona.codigo_agente : '0').trim();
    if (agentCodeRaw === '' || agentCodeRaw === 'null' || agentCodeRaw === 'undefined' || agentCodeRaw === '0') {
      agentCodeRaw = '0';
    }
    
    // El mapeo garantiza que '0' -> 'GRAN CANARIA'
...
const ClientColumn: React.FC<{ data: any; darkMode: boolean }> = ({ data, darkMode }) => (
  <div className={`flex flex-col flex-1 min-w-0 h-full border-r last:border-r-0 transition-all ${darkMode ? 'bg-slate-950 border-white/5' : 'bg-white border-gray-200'}`}>
    {/* Cabecera compacta para ahorrar espacio vertical */}
    ...
    {/* Lista de productos con tabla ultra-compacta */}
    ...
    {/* Subtotal por cliente con tipografía masiva para visibilidad lejana */}
    <div className={`px-6 py-8 mt-auto border-t-2 ${darkMode ? 'bg-red-600/5 border-red-600/20' : 'bg-red-50 border-red-200'}`}>
      <div className="flex flex-col items-end">
        ...
        <span className="text-6xl xl:text-8xl font-black text-red-600 leading-none tabular-nums tracking-tighter">
          {data.grandTotal.toLocaleString('es-ES', { minimumFractionDigits: 0, maximumFractionDigits: 2 })}
        </span>
      </div>
    </div>
  </div>
...
function App() {
  ...
  return (
    <div className={`flex flex-col h-screen w-screen overflow-hidden ${darkMode ? 'bg-slate-950' : 'bg-gray-100'}`}>
      {/* Header Industrial a ancho completo */}
        ...
        <div className="flex justify-between items-center w-full">
          <div className="flex items-center gap-6">
            ...
            <div>
              ...
              <p className="text-[10px] font-black text-gray-500 uppercase tracking-[0.6em] mt-1">SISTEMA CENTRAL • 4K ULTRA • FULL WIDTH</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-10">
            <div className="flex flex-col items-end px-10 border-r-2 border-white/10">
               ...
               <span className="text-7xl font-black text-red-600 tabular-nums leading-none tracking-tighter">
                {totalPlanta.toLocaleString('es-ES', { minimumFractionDigits: 0, maximumFractionDigits: 2 })}
               </span>
            </div>
      ...
      {/* Grid Fluido de Clientes - Ocupa todo el espacio horizontal sin scroll */}
      <main className="flex-1 w-full flex overflow-hidden">
        {clientGroups.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center gap-6 animate-fade-in opacity-30">
            ...
            <h2 className="text-4xl font-black uppercase tracking-[0.8em]">ESPERANDO DATOS DE MAKE...</h2>
          </div>
        ) : (
